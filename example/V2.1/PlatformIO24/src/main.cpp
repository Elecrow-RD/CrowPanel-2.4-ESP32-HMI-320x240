...
// Retains the buffer allocation used by the original PlatformIO project.
...
// Maps raw touch readings to the 2.4-inch panel coordinates.
...
{
    // LVGL uses inclusive coordinates for both corners of the dirty area.
    uint32_t w = (area->x2 - area->x1 + 1);
    uint32_t h = (area->y2 - area->y1 + 1);

    // Transfer this partial-rendered rectangle to the LCD.
    lcd.startWrite();
    lcd.setAddrWindow(area->x1, area->y1, w, h);
    lcd.pushColors(reinterpret_cast<uint16_t *>(px_map), w * h, true);
    lcd.endWrite();

    // Release the LVGL buffer only after the LCD has consumed its pixels.
 ...
 *
 * LVGL polls this callback while processing input. Coordinates are passed
 * through unchanged because the calibration data matches rotation 1.
 *
...
{
    (void)indev;
    // 600 filters out low-pressure noise while accepting a normal press.
    bool touched = lcd.getTouch(&touchX, &touchY, 600);

    if(!touched) {
        // Explicitly report release so LVGL can finish the click event.
        data->state = LV_INDEV_STATE_RELEASED;
    }
    else {
        // Pass the pressed state and calibrated coordinates to LVGL.
